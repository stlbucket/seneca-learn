...
describe(__filename, function() {
	...
	it('multi-step pipe - explicit execute', function (done) {
		...
		pipe.execute({
			...
			.then(function (result) {
				vos().clog('MULTI-STEP PIPE WORKSPACE', pipe.ws, true);
				vos().clog('EXPLICIT EXECUTE MULTI-STEP PIPE RESULT', result, true);
	...
	it('multi-step pipe - implicit execute', function (done) {
		...
		pipeDef({
			...
			.then(function (result) {
				vos().clog('IMPLICIT EXECUTE MULTI-STEP PIPE RESULT', result, true);
				result.testId.should.equal(testId);
				done();
			});
	...
	it('multi-step pipe - curried execute', function (done) {
		...
		pipe({
			...
			.then(function (result) {
				vos().clog('CURRIED EXECUTE MULTI-STEP PIPE RESULT 1', result, true);
				result.testId.should.equal(testId);
			...
			.then(function (result) {
				vos().clog('CURRIED EXECUTE MULTI-STEP PIPE RESULT 2', result, true);
...
// D:\git\vos\scripts\core\vosPipe\specPipes\multiStep\spec.js
...
// 													filename: 'D:\\git\\vos\\scripts\\core\\vosPipe\\specPipes\\oneStep\\index.js',
